...
server
  ...
  .then(() => {
    ...
    db.run(
      "CREATE TABLE contact_forms (contact_form_id INTEGER PRIMARY KEY,name TEXT,email TEXT,message TEXT);",
      (err) => {
        if (err) {
    ...
    const getCoords = async (location) => {
      ...
      let boundingBox =
        "&boundingBox=39.112823,-77.164707,38.581269,-76.496682";
      let locationURL =
    ...
    async function processDataForFrontEnd() {
      const baseURL =
        "https://data.princegeorgescountymd.gov/resource/9hyf-46qb.json"; // Enter the URL for the data you would like to retrieve here
          ...
          parsedResponse.forEach((entry) => {
            try {
              if (entry.property_id in addressDict) {
                ...
                if (
                  entry.violation_id in
                  addressDict[entry.property_id].violations
                ) {
                  console.log("Look at entry: ", entry);
                } else {
                  addressDict[entry.property_id].violations.push({
                    violationID: entry.violation_id,
                    inspectionID: entry.inspection_id,
                    code: entry.violation_code,
                    desc: entry.violation_description,
                  });
                }
              } else {
                let address = {
                  ...
                  street_suffix: entry.street_type,
                  city: entry.city,
                  state: entry.state,
                  zip: entry.zip_code,
                };
                addressDict[entry.property_id] = {
                  address: address,
                  count: 1,
                  violations: [
                    {
    ...
    app.post("/api/form/submit", (req, res) => {
      ...
      db.run(
        "INSERT INTO contact_forms(name, email, message) VALUES(?,?,?);",
        [
          req.session.form.name,
          req.session.form.email,
          req.session.form.message,
        ],
        (err) => {
          if (err) {
            return console.log(err.message);
